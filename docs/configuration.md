...
Semantic `blocked` state for Ask User and Plan Mode dialogs requires the current official Herdr Pi integration. Run `herdr integration install pi` to install or update it. Run `herdr integration status` to verify it. The official `herdr:pi` integration remains the lifecycle authority.
