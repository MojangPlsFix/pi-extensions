...
- Subagents resolve model policy when they start. Resolution checks an explicit review override, `models.overrides`, profile frontmatter, `models.default`, and the parent snapshot. `inherit` selects the parent value.

### Subagent model providers

Subagent model IDs include the provider name. Child sessions use the selected model through Pi's model runtime. They do not run the `copilot` executable.
...
- `PI_CODING_AGENT_DIR`: Pi's agent directory. User Subagent profiles and configuration live below this directory. New Subagent transcripts use `<agent-dir>/subagents/sessions`.
- `PI_CODING_AGENT_SESSION_DIR`: the normal Pi session location used by Stats. Stats also scans legacy session trees.
- `HERDR_ENV=1`, `HERDR_PANE_ID`, and `HERDR_SOCKET_PATH`: permit optional display-only Subagent transcript panes. Herdr does not run child agents.
...
- **Subagent capabilities:** The user catalog can load trusted extensions, skills, and external command rules. Profiles select capability names. Project profiles cannot define implementation paths.
- **Herdr:** Subagents can open a raw display pane for a persisted child transcript. The Agent Hub remains the lifecycle authority.
...
Subagents v2 uses `~/.pi/agent/subagents/config.json`, or the matching path under `PI_CODING_AGENT_DIR`.
...
Run `/agents doctor --json` to inspect effective profile and capability policy. Read [the Subagents documentation](../packages/subagents/README.md) for the full schema and migration steps.
