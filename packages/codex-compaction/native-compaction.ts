...
function responsesCompat(model: Model<any>): {
...
function isRetainedUserContentPart(value: unknown): boolean {
  if (!isJsonObject(value)) return false;
  if (value.type === "input_text") return typeof value.text === "string" && value.text.length > 0;
  return (
...
function isRetainedUserItem(value: unknown): value is ResponseItem {
  ...
  return (
    ...
    value.content.every(isRetainedUserContentPart)
  );
}

function isOpaqueCompactionItem(value: unknown): value is ResponseItem {
  return (
    isJsonObject(value) &&
...
export function parseNativeCompactionDetails(value: unknown): NativeCompactionDetails | undefined {
  ...
  if (value.replacementHistory.length === 0) return undefined;
  const compactionItem = value.replacementHistory.at(-1);
  const retainedUsers = value.replacementHistory.slice(0, -1);
  if (!isOpaqueCompactionItem(compactionItem) || !retainedUsers.every(isRetainedUserItem)) {
    return undefined;
  }
  ...
  return {
    ...
    replacementHistory: value.replacementHistory.map((item) => cloneItem(item as ResponseItem)),
  };
}
...
function normalizeResponseToolCallId(
  id: string,
  targetModel: Model<any>,
  source: AssistantMessage,
): string {
  if (!CODEX_TOOL_CALL_PROVIDERS.has(targetModel.provider) || !id.includes("|")) {
    return normalizeIdPart(id);
  }
  const [callId = id, itemId = ""] = id.split("|");
  const normalizedCallId = normalizeIdPart(callId);
  const isForeign = source.provider !== targetModel.provider || source.api !== targetModel.api;
  let normalizedItemId = isForeign ? `fc_${shortHash(itemId)}` : normalizeIdPart(itemId);
  if (!normalizedItemId.startsWith("fc_")) normalizedItemId = `fc_${normalizedItemId}`;
  return `${normalizedCallId}|${normalizeIdPart(normalizedItemId)}`;
}
...
function transformMessagesForCodex(messages: Message[], model: Model<any>): Message[] {
  ...
  const transformed = imageAwareMessages.map((message): Message => {
    ...
    const content = message.content.flatMap((block): AssistantMessage["content"] => {
      ...
      if (!isSameModel && block.thoughtSignature) {
        normalizedToolCall = { ...block, thoughtSignature: undefined };
      }
      if (!isSameModel) {
        const normalizedId = normalizeResponseToolCallId(block.id, model, message);
...
function messagesToResponseItems(
  ...
  let messageIndex = 0;
  const transformedMessages = transformMessagesForCodex(messages, model);

  for (const message of transformedMessages as unknown as JsonObject[]) {
    if (message.role === "user") {
      ...
      for (const block of message.content) {
        ...
        if (block.type === "thinking" && typeof block.thinkingSignature === "string") {
          try {
            ...
            if (isJsonObject(reasoning) && reasoning.type === "reasoning")
              items.push(cloneItem(reasoning));
          } catch {}
          continue;
        }
        ...
        if (block.type === "toolCall" && typeof block.id === "string") {
          ...
          const callId = rawCallId || block.id;
          pendingToolCalls.set(block.id, callId);
          items.push({
            type: "function_call",
            call_id: callId,
            ...(!isDifferentModel && normalizedItemId(rawItemId)
              ? { id: normalizedItemId(rawItemId) }
              : {}),
      ...
      const addedTools = Array.isArray(message.addedToolNames)
        ...
        : [];
      if (addedTools.length > 0 && responsesCompat(model).supportsToolSearch === true) {
        ...
        items.push({
          ...
          status: "completed",
          arguments: {
            query: addedTools.map((tool) => tool.name).join(" "),
            limit: addedTools.length,
          },
        });
        items.push({
          ...
          tools: addedTools.map((tool) =>
            responseTool(tool, true, responsesCompat(model).supportsStrictMode !== false),
          ),
        });
      }
    }
...
export function effectiveInputForBranch(params: {
  ...
  if (params.excludeLastAssistantError) {
    ...
    if (lastAssistantIndex >= 0) {
      const failedEntry = branch[lastAssistantIndex];
      const failedToolCallIds = new Set(
        failedEntry?.type === "message" && failedEntry.message.role === "assistant"
          ? failedEntry.message.content.flatMap((block) =>
              block.type === "toolCall" ? [block.id] : [],
            )
          : [],
      );
      branch = branch.filter((entry, index) => {
        if (index === lastAssistantIndex) return false;
        return !(
          index > lastAssistantIndex &&
          entry.type === "message" &&
          entry.message.role === "toolResult" &&
          failedToolCallIds.has(entry.message.toolCallId)
        );
      });
    }
  }
...
function isImagePart(value: unknown): value is JsonObject {
  return (
    isJsonObject(value) &&
    value.type === "input_image" &&
    typeof value.image_url === "string" &&
...
function hasRetainableUserContent(item: ResponseItem): boolean {
  if (responseItemText(item).trim()) return true;
  return Array.isArray(item.content) && item.content.some(isImagePart);
}

function approximateTokens(item: ResponseItem): number {
  const imageTokens = Array.isArray(item.content)
    ? item.content.filter(isImagePart).length * ESTIMATED_IMAGE_TOKENS
    : 0;
  return Math.max(1, Math.ceil(responseItemText(item).length / 4) + imageTokens);
}
...
function truncateMessage(item: ResponseItem, maxTokens: number): ResponseItem | undefined {
  ...
  let remainingImageTokens = maxTokens;
  const retainedImageIndexes = new Set<number>();
  for (let index = content.length - 1; index >= 0; index--) {
    if (!isImagePart(content[index]) || remainingImageTokens < ESTIMATED_IMAGE_TOKENS) continue;
    retainedImageIndexes.add(index);
    remainingImageTokens -= ESTIMATED_IMAGE_TOKENS;
  }
  remainingCharacters = remainingImageTokens * 4;
  ...
  let consumed = 0;
  const truncatedContent = content.flatMap((part, index) => {
    if (isImagePart(part)) return retainedImageIndexes.has(index) ? [part] : [];
    if (!isJsonObject(part) || typeof part.text !== "string") return [part];
    const remainingText = totalText - consumed;
    const partBudget =
      remainingText === 0
        ? 0
        : Math.floor((part.text.length / remainingText) * remainingCharacters);
...
export function buildReplacementHistory(
  ...
  if (
    ...
    !compactionItem.encrypted_content.trim()
  ) {
  ...
  return tools.length > 0
    ? tools.map((tool) =>
        responseTool(
          tool,
          false,
          model ? responsesCompat(model).supportsStrictMode !== false : true,
        ),
      )
    : undefined;
}
...
export function buildCompactionRequestBody(params: {
  ...
  const body: JsonObject = {
    ...
    input: [
      ...params.input.filter((item) => item.type !== "compaction_trigger").map(cloneItem),
...
async function parseSseResponse(
  ...
  const processBlock = (block: string) => {
    ...
    if (event.type === "error") {
      const nestedError = isJsonObject(event.error) ? event.error : undefined;
      const message =
    ...
    if (event.type === "response.failed") {
      ...
      const nestedError = isJsonObject(response?.error) ? response.error : undefined;
      const message =
        typeof nestedError?.message === "string" && nestedError.message.trim()
          ? nestedError.message
    ...
    if (event.type === "response.completed" || event.type === "response.done") {
      ...
      if (status === "incomplete" || status === "queued" || status === "in_progress") {
        throw new RetryableCompactionStreamError(
          `OpenAI Codex compaction ended with response status ${status}.`,
        );
      }
      ...
      if (status !== undefined && status !== "completed") {
        throw new NonRetryableCompactionError(
          `OpenAI Codex compaction ended with unknown response status ${status}.`,
        );
      }
      completed = true;
  ...
  if (typeof item.encrypted_content !== "string" || !item.encrypted_content.trim()) {
    throw new NonRetryableCompactionError(
      "OpenAI Codex returned a compaction item without non-empty encrypted_content.",
    );
  }
  return { item, usage };
}
...
export async function callRemoteCompaction(params: {
  ...
  const retryDelay = params.delay ?? waitForRetry;
  ...
  for (let attempt = 0; attempt <= MAX_REMOTE_RETRIES; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        lastError = error;
        await retryDelay(parseRetryDelay(response) ?? 1000 * 2 ** attempt, params.signal);
        continue;
      }
      ...
      lastError = error;
      if (attempt === MAX_REMOTE_RETRIES) throw error;
      await retryDelay(1000 * 2 ** attempt, params.signal);
    }
  }
