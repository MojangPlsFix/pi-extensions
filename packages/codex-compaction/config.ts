...
export const DEFAULT_CONFIG: CodexCompactionConfig = {
  autoCompact: true,
  thresholdRatio: 0.9,
};

export function readConfig(path: string): Partial<CodexCompactionConfig> {
  if (!existsSync(path)) return {};
  try {
    const parsed = JSON.parse(readFileSync(path, "utf8")) as Record<string, unknown>;
    return {
      ...(typeof parsed.autoCompact === "boolean" ? { autoCompact: parsed.autoCompact } : {}),
      ...(typeof parsed.thresholdRatio === "number" &&
      parsed.thresholdRatio > 0 &&
      parsed.thresholdRatio < 1
...
export function loadConfig(cwd: string, projectTrusted: boolean): CodexCompactionConfig {
  const paths = resolveConfigPaths(cwd, projectTrusted);
  const globalConfig = readConfig(paths.globalPath);
  const projectConfig = paths.projectPath ? readConfig(paths.projectPath) : {};
