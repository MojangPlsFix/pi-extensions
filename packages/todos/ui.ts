...
import { renderAssignmentSuffix } from "./formatters.js";
import {
  filterTodos,
  formatTodoId,
  isTodoClosed,
...
export class TodoSelectorComponent extends Container implements Focusable {
  ...
  constructor(
    ...
    private onQuickAction?: (todo: TodoFrontMatter, action: TodoShortcutAction) => void,
  ...
  private updateHints(): void {
    this.hintText.setText(
      this.theme.fg(
        "dim",
        "Type to search • ↑↓ select • Enter actions • Shift+A add • Shift+W work • Shift+R refine • Shift+E edit • Shift+C close/reopen • Shift+D delete • Esc close",
      ),
    );
  }
  ...
  handleInput(keyData: string): void {
    ...
    if (matchesKey(keyData, "shift+a")) {
      this.onCreate?.();
      return;
    }
    if (matchesKey(keyData, "shift+r")) {
      const selected = this.filteredTodos[this.selectedIndex];
      if (selected && this.onQuickAction) this.onQuickAction(selected, "refine");
      return;
    }
    if (matchesKey(keyData, "shift+w")) {
      const selected = this.filteredTodos[this.selectedIndex];
      if (selected && this.onQuickAction) this.onQuickAction(selected, "work");
      return;
    }
    if (matchesKey(keyData, "shift+e")) {
      const selected = this.filteredTodos[this.selectedIndex];
      if (selected && this.onQuickAction) this.onQuickAction(selected, "edit");
      return;
    }
    if (matchesKey(keyData, "shift+v")) {
      const selected = this.filteredTodos[this.selectedIndex];
      if (selected && this.onQuickAction) this.onQuickAction(selected, "view");
      return;
    }
    if (matchesKey(keyData, "shift+c")) {
      const selected = this.filteredTodos[this.selectedIndex];
      if (selected && this.onQuickAction) this.onQuickAction(selected, "toggleClosed");
      return;
    }
    if (matchesKey(keyData, "shift+d")) {
      const selected = this.filteredTodos[this.selectedIndex];
      if (selected && this.onQuickAction) this.onQuickAction(selected, "delete");
      return;
    }
...
export class TodoActionMenuComponent extends Container {
  ...
  private onCancelCallback: () => void;
  private closeAction: "close" | "reopen";
  ...
  constructor(
    ...
    this.closeAction = closed ? "reopen" : "close";
    this.hasRelease = Boolean(todo.assigned_to_session);
    const options: SelectItem[] = [
      ...
      { value: "edit", label: "edit", description: "Edit title, tags, and details" },
      ...(closed
    ...
    this.addChild(
      new Text(
        theme.fg(
          "dim",
          "Enter confirm • Esc back • v view • w work • r refine • e edit • c close/reopen • l release • p path • y text • d delete",
        ),
      ),
    );
    this.addChild(new DynamicBorder((s: string) => theme.fg("accent", s)));
  ...
  handleInput(keyData: string): void {
    if (matchesKey(keyData, "v")) {
      this.onSelectCallback("view");
      return;
    }
    if (matchesKey(keyData, "w")) {
      this.onSelectCallback("work");
      return;
    }
    if (matchesKey(keyData, "r")) {
      this.onSelectCallback("refine");
      return;
    }
    if (matchesKey(keyData, "e")) {
      this.onSelectCallback("edit");
      return;
    }
    if (matchesKey(keyData, "c")) {
      this.onSelectCallback(this.closeAction);
      return;
    }
    if (matchesKey(keyData, "l") && this.hasRelease) {
      this.onSelectCallback("release");
      return;
    }
    if (matchesKey(keyData, "p")) {
      this.onSelectCallback("copyPath");
      return;
    }
    if (matchesKey(keyData, "y")) {
      this.onSelectCallback("copyText");
      return;
    }
    if (matchesKey(keyData, "d")) {
...
export class TodoDeleteConfirmComponent extends Container {
  ...
  constructor(theme: Theme, message: string, onConfirm: (confirmed: boolean) => void) {
    ...
    this.addChild(this.selectList);
    this.addChild(new Text(theme.fg("dim", "Enter confirm • y yes • n no • Esc back")));
    this.addChild(new DynamicBorder((s: string) => theme.fg("accent", s)));
  }
