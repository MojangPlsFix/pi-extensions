...
External MCP calls have no upstream cancellation field. On abort, Pi stops waiting and renders a cancelled state, but the adapter explicitly does **not** claim that the external operation was killed. Fetching and indexing cross the external runtime boundary and should be given only explicitly selected sources/content; indexed or fetched text remains untrusted data, not instructions. The bridge lifecycle is shut down on session shutdown and inherits the external bridge's pending-request, respawn, EPIPE, output, and process-tree protections.
...
The replacement composes the pinned upstream Pi lifecycle through a tool-suppressing adapter. Session event capture, active-memory injection, resume snapshots, compaction counters, token/cost tracking, and the upstream stats/doctor commands remain active, while the upstream MCP bridge is forced off.

After each upstream context handler settles, it rebuilds the routing paragraph from one `pi.getActiveTools()` snapshot. The guidance names only tools active for that turn. Exact file retrieval routes to Pi's built-in `read` when active. Context Mode does not register a `ctx_read` tool. `ctx_execute_file` runs caller-supplied code over one file. It is not a read-only replacement for `read`. Fetch-to-search guidance appears only when both tools are active. Index, statistics, and diagnostics guidance appears only when each related tool is active. If active-tool lookup fails, the paragraph becomes exactly `context-mode active.` Content after that paragraph remains byte-for-byte identical.

Plan Mode blocks `ctx_execute`, `ctx_execute_file`, and `ctx_batch_execute`, even if another extension reactivates them. The next agent turn removes these execution tools again. Outside Plan Mode, active Context execution tools keep their normal behavior. Plan Mode keeps `ctx_index` and `ctx_fetch_and_index` available for compatibility. These tools can change external Context state, and fetch also uses the network.
